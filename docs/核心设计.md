...
Agent 可以领取并执行开发。Agent 通过接口领取后，任务会自动进入
`Code Implementation` 阶段。
...
代码实现阶段完全由 Agent 控制，不提供人工开放、审批或手动推进操作。
Agent 完成开发后，通过接口提交执行结果和 Git Commit SHA，任务进入
代码审核阶段。
...
该阶段仅由人工执行，不开放 Agent 领取。测试通过并由人工确认后，任务进入归档阶段。测试
失败时，同一任务返回技术拆解阶段并标记为 `Need Redo`，等待人工补充上下文
并再次开放给 Agent。

### 6. 归档

归档是任务的最终阶段。进入该阶段后任务标记为已完成，不再开放 Agent 领取或人工流转操作。

...
function DetailRow({ label, testID, value }: { label: string; testID: string; value: string }) {
  return (
    <div className="grid gap-1" data-testid={`${testID}-row`}>
      <dt className="text-xs font-medium text-muted-foreground" data-testid={`${testID}-label`}>{label}</dt>
      <dd className="text-sm" data-testid={`${testID}-value`}>{value}</dd>
    </div>
  )
}
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent data-testid={`task-detail-dialog-${task.ID}`}>
        ...
        <dl className="grid gap-4 sm:grid-cols-2" data-testid={`task-detail-fields-${task.ID}`}>
          ...
          <div className="grid gap-1" data-testid={`task-detail-agent-ready-${task.ID}-row`}>
            <dt className="text-xs font-medium text-muted-foreground" data-testid={`task-detail-agent-ready-${task.ID}-label`}>Agent 执行</dt>
            <dd data-testid={`task-detail-agent-ready-${task.ID}-value`}>
              <Badge data-testid={`task-detail-agent-ready-${task.ID}`} variant={task.AgentReady ? 'secondary' : 'outline'}>{task.AgentReady ? '可执行' : '不可执行'}</Badge>
            </dd>
          </div>
        </dl>
        <div className="grid gap-3" data-testid={`task-detail-agent-work-${task.ID}`}>
          ...
          <div className="grid gap-2" data-testid={`task-detail-agent-run-list-${task.ID}`}>
            {agentWork.data?.Runs.length ? agentWork.data.Runs.map((run) => (
              <div className="rounded-md border px-3 py-2 text-sm" data-testid={`task-detail-agent-run-${task.ID}-${run.ID}`} key={run.ID}>
                ...
                <p data-testid={`task-detail-agent-run-result-${run.ID}`}>{run.Result}</p>
                {run.Passed == null ? null : <Badge data-testid={`task-detail-agent-run-verdict-${run.ID}`} variant={run.Passed ? 'secondary' : 'outline'}>{run.Passed ? '审核通过' : '审核不通过'}</Badge>}
              </div>
          ...
          <div className="grid gap-2" data-testid={`task-detail-human-review-list-${task.ID}`}>
            ...
            )) : <p className="text-sm text-muted-foreground" data-testid={`task-detail-human-review-empty-${task.ID}`}>暂无人工审核记录</p>}
          </div>
        </div>
        <div className="grid gap-3" data-testid={`task-detail-refs-${task.ID}`}>
          <h3 className="text-sm font-medium" data-testid={`task-detail-refs-title-${task.ID}`}>引用任务</h3>
          <div className="flex gap-2" data-testid={`task-detail-ref-controls-${task.ID}`}>
            <select className="h-9 min-w-0 flex-1 rounded-md border bg-background px-3 text-sm" data-testid={`task-detail-ref-select-${task.ID}`} onChange={(event) => setReferencedTaskID(event.target.value)} value={referencedTaskID}>
              <option data-testid={`task-detail-ref-option-empty-${task.ID}`} value="">选择同项目任务</option>
              {candidates.map((candidate) => <option data-testid={`task-detail-ref-option-${task.ID}-${candidate.ID}`} key={candidate.ID} value={candidate.ID}>{candidate.Title}</option>)}
            </select>
            <Button data-testid={`task-detail-ref-add-${task.ID}`} disabled={!referencedTaskID || addRef.isPending} onClick={() => addRef.mutate()} type="button">添加引用</Button>
          </div>
          <div className="grid gap-2" data-testid={`task-detail-ref-list-${task.ID}`}>
            {refs.data?.length ? refs.data.map((ref) => <div className="rounded-md border px-3 py-2 text-sm" data-testid={`task-detail-ref-${task.ID}-${ref.ID}`} key={ref.ID}>{ref.Title}</div>) : <p className="text-sm text-muted-foreground" data-testid={`task-detail-ref-empty-${task.ID}`}>暂无引用任务</p>}
          </div>
        </div>
      </DialogContent>
    </Dialog>

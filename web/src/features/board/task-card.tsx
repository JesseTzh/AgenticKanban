...
export function TaskCard({ projectID, task }: { projectID: string; task: Task }) {
  ...
  return (
    <Card
      ...
      className="cursor-pointer gap-3 py-4 transition-[background-color,box-shadow] hover:bg-surface-bright hover:shadow-button-hover"
    ...
    >
      ...
      <CardContent className="px-4" data-test-id={`task-card-content-${task.ID}`}>
      ...
      <CardFooter className="justify-end px-4" data-test-id={`task-card-footer-${task.ID}`}>
        <DropdownMenu>

...
export function AgentKeysPage() {
  ...
  return (
    <AdminShell title="Agent 密钥">
      <section className="grid gap-6" data-testid="agent-keys-page">
        <header className="flex items-start justify-between gap-4" data-testid="agent-keys-heading">
          <div data-testid="agent-keys-heading-copy">
            <h2 className="text-2xl font-semibold" data-testid="agent-keys-title">Agent 密钥</h2>
            <p className="text-sm text-muted-foreground" data-testid="agent-keys-description">为 Agent 创建独立访问密钥，系统会按密钥记录任务执行来源。</p>
          </div>
        ...
        {keys.data?.length ? (
          <Table data-testid="agent-keys-table">

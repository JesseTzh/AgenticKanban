...
AgenticKanban 是一个以 Kanban 为核心驱动的 Agentic Coding 项目管理系统，用于将需求提出、技术拆解、代码实现、代码审查、人工测试和完成归档串联为一套标准化流程，使人类与 Agent 能够围绕同一套任务状态协同工作。
...
开发完成后，需要将对应的 Git 提交记录关联到技术任务，由此形成代码审查任务。

本阶段状态定义：

- `Not Ready`：缺少有效 Git 提交记录，或提交内容与技术任务无法明确对应。
- `Agentic Ready`：代码提交已关联，变更内容清晰，具备进入人工或 Agent 代码审查的条件。
...
测试不通过时，任务将被打回技术拆解阶段，同时原代码审查任务被锁定，直到新的 Git 提交记录进入审查流程。

本阶段状态定义：

- `Not Ready`：测试条件不足，缺少可验证版本，或相关代码审查尚未完成。
